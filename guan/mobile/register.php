...
switch($action) {
	...
	case 'send':
		...
		$t = $db->get_one("SELECT mobile,groupid FROM {$DT_PRE}member WHERE mobile='$mobile'");
		if($t) exit('3');
	...
	case 'post':
		...
		if(!preg_match("/^[0-9]{6}$/",$code) || $_SESSION['mobile_code'] != md5($mobile.'|'.$code)) exit('ko');
		$post = array();
		$post['regid'] = isset($regid) ? intval($regid) : 0;
		// 验证手机是否存在
		$url = 'http://192.168.100.116/ajax.php?action=laoqumember';
		$data = array('mobile'=>$mobile,'check'=>1);
		$res = tongbu($url,$data);
		if($res != 1){
			dalert('该手机号已被注册','goback');
			exit();
		}

		$username = $post['username'] = 'm'.$mobile;
		...
		if($do->add($post)) {
			// 同步会员
			$userinfo['member'] = $db->get_one("SELECT * FROM {$DT_PRE}member WHERE userid='$do->userid'");
			$userinfo['company'] = $db->get_one("SELECT * FROM {$DT_PRE}company WHERE userid='$do->userid'");
			$url = 'http://192.168.100.116/ajax.php?action=laoqumember';
			...
			if($MOD['welcome_sms'] && is_mobile($mobile)) {
				$message = lang('sms->wel_reg', array($post['truename'], $DT['sitename'], $post['username'], $post['password']));
				$message = strip_sms($message);
