...
switch($action) {
	case 'favorite':
		...
		$res = $db->get_one("SELECT * FROM {$DT_PRE}favorite WHERE userid=$_userid AND typeid=$itemid");
		if($res){
			$db->query("DELETE FROM {$DT_PRE}favorite WHERE userid=$_userid AND typeid=$itemid");
			echo 0;
		}else{
			$time = time();
			$db->query("INSERT INTO {$DT_PRE}favorite(listorder,userid,typeid,addtime)VALUES($orders,$_userid,$itemid,$time)");
			echo 1;
		}
	break;
	case 'comment':
