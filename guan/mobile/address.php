...
switch($action) {
	case 'add':
		if(isset($_POST['ok'])){
			...
			if($post['checkbox1']){
				$post['note'] = 1;
				$db->query("UPDATE {$DT_PRE}address SET note = 0 WHERE username='$_username'");
			}
			if($do->pass($post)) {
	...
	default:
		...
		$lists = array();
		if($items) {
			$result = $db->query("SELECT * FROM {$DT_PRE}address WHERE $condition ORDER BY note DESC,itemid ASC LIMIT $offset,$pagesize");
