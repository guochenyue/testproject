...
function get_area($areaid) {
	...
	$res = $db->get_one("SELECT * FROM {$db->pre}area WHERE areaid=$areaid");
	$res['countgoods'] = $db->get_one("SELECT count(*) FROM {$db->pre}mall WHERE areaid=$areaid")['count(*)'];
	$res['hits'] = $db->get_one("SELECT hits FROM {$db->pre}company WHERE areaid=$areaid")['hits'];
	$res['company'] = $db->get_one("SELECT company FROM {$db->pre}company WHERE areaid=$areaid")['company'];
	$res['fuchi'] = $db->get_one("SELECT fuchi FROM {$db->pre}company WHERE areaid=$areaid")['fuchi'];
	return $areaid ? $res : array();
}
...
function get_catid($catid) {
	...
	if(!is_numeric($catid)) return array();
	$catid = intval($catid);
	return $catid ? $db->get_one("SELECT arrchildid FROM {$db->pre}category WHERE catid=$catid") : array();
}
function get_catids($catid) {
	global $db;
	if(!is_numeric($catid)) return array();
	$catid = intval($catid);
	$result = $db->get_one("SELECT arrchildid FROM {$db->pre}category WHERE catid=$catid");
	 $ids = implode(',',$result);
    return $ids;
}
function get_jd_catids($catid) {
	global $db;
	if(!is_numeric($catid)) return array();
	$catid = intval($catid);
	$result = $db->get_one("SELECT arrchildid FROM {$db->pre}category WHERE catid=$catid");
...
function get_goods($catid,$limit){
	global $db;
	$result = get_catid($catid);	
    $ids = implode(',',$result);
    //return $ids;
    $sql = "SELECT * FROM {$db->pre}mall WHERE catid in ($ids) and level <> 4 order by rand() desc limit $limit";
    $result = $db->query("$sql",'CACHE');
    $pos = array();
    while($r = $db->fetch_array($result)) {
    	// $r['thumb'] = str_replace('http://www.telq.com/','http://192.168.100.146/',$r['thumb']);//替换ip
    	$r['areaid'] = area_pos($r['areaid']);
		$r['thumb'] = str_replace('.thumb.'.file_ext($r[thumb]), '',$r[thumb]);
		$pos[] = $r;
	}
	return $pos;
}
function get_ts_goods($catid,$username,$limit){
	global $db;
	$result = get_catid($catid);	
    $ids = implode(',',$result);
    //return $ids;
    $sql = "SELECT * FROM {$db->pre}mall WHERE catid in ($ids)  AND level=3  AND username='$username' AND status=3 order by addtime desc limit $limit";
    $result = $db->query("$sql",'CACHE');
    $pos = array();
    while($r = $db->fetch_array($result)) {
		$pos[] = $r;
...
function get_ms_goods(){
	global $db;
	$date = time();
    $sql = "SELECT * FROM {$db->pre}mall WHERE totime > '$date' AND status=3 order by addtime desc ";
    $result = $db->query("$sql",'CACHE');
    $pos = array();
    while($r = $db->fetch_array($result)) {
		$pos[] = $r;
...
function get_good($catid,$username){
	global $db;
	$result = get_catid($catid);	
    $ids = implode(',',$result);
    //return $ids;
    $sql = "SELECT * FROM {$db->pre}mall WHERE catid in ($ids) AND username='$username' AND status=3 order by addtime desc";
    $result = $db->query("$sql",'CACHE');
    $pos = array();
    while($r = $db->fetch_array($result)) {
		$pos[] = $r;
...
function get_goodss($catid){
	global $db;
	$result = get_catid($catid);	
    $ids = implode(',',$result);
    //return $ids;
    $sql = "SELECT * FROM {$db->pre}mall WHERE catid in ($ids) AND level=3 AND status=3 order by addtime desc LIMIT 6";
    $result = $db->query("$sql",'CACHE');
    $pos = array();
    while($r = $db->fetch_array($result)) {
		$pos[] = $r;
...
function get_changguan($username) {
	global $db;
	return $username ? $db->get_one("SELECT changguan FROM {$db->pre}company WHERE username='$username'") : array();
}
function get_tjg() {
	global $db;
	return  $db->get_one("SELECT * FROM {$db->pre}company WHERE level=2");
}
function get_introduce($userid) {
	global $db;
	if(!is_numeric($userid)) return array();
	$userid = intval($userid);
	return $userid ? $db->get_one("SELECT content  FROM {$db->pre}company_data as a LEFT JOIN {$db->pre}company as b ON  a.userid = b.userid WHERE a.userid=$userid") : array();
}
function area_post($areaid, $str = ' · ', $deep = 0) {
	$areaid = intval($areaid);
	if($areaid) {
		global $AREA;
	} else {
...
function get_ly_maincat($catid, $moduleid, $level = -1) {
	global $db;
	$catid = intval($catid);
	$condition = $catid ? "parentid=$catid" : "moduleid=$moduleid AND parentid=0";
	if($level >= 0) $condition .= " AND level=$level";
	$cat = array();
	$result = $db->query("SELECT catid,catname,child,style,linkurl,item FROM {$db->pre}category WHERE $condition ORDER BY listorder,catid ASC LIMIT 5", 'CACHE');
	while($r = $db->fetch_array($result)) {
		$cat[] = $r;
	}
	return $cat;
...
function dt_com($url){
   ...
   if(strstr($url,"/0")){
       ...
       if(strstr($url,"http:/1")){
           $url = str_replace('http:/1', 'http://1', "$url");
       }elseif(strstr($url,"http:/0")){
           $url = str_replace('http:/0', 'http://0', "$url");
       }
   }
   return $url;
}
function get_tsp_goods(){
	global $db;
    $sql = "SELECT * FROM {$db->pre}mall WHERE level = 1 order by addtime desc limit 10";
    $result = $db->query("$sql",'CACHE');
    $pos = array();
    while($r = $db->fetch_array($result)) {
    	$r['areaid'] = area_pos($r['areaid']) . '老区特色馆';
		$r['thumb'] = str_replace('.thumb.'.file_ext($r[thumb]), '',$r[thumb]);
		$pos[] = $r;
	}
	return $pos;
}
function get_tsp_com(){
	global $db;
    $sql = "SELECT * FROM {$db->pre}company WHERE level = 1 and groupid<>1  limit 6";
    $result = $db->query("$sql",'CACHE');
    $pos = array();
    while($r = $db->fetch_array($result)) {
    	$r['areaid'] = area_pos($r['areaid']) . '特产';
		$pos[] = $r;
	}
	return $pos;
}
function get_tsp_cg(){
	global $db;
    $sql = "SELECT * FROM {$db->pre}company WHERE level = 1 and groupid<>1  limit 9";
    $result = $db->query("$sql",'CACHE');
    $pos = array();
    while($r = $db->fetch_array($result)) {
    	$r['areaid'] = area_pos($r['areaid']) . '特产';
...
function get_ts_changguan(){
	global $db;
    $sql = "SELECT * FROM {$db->pre}company WHERE level = 3 and groupid<>1  limit 6";
    $result = $db->query("$sql",'CACHE');
    $pos = array();
    while($r = $db->fetch_array($result)) {
		$pos[] = $r;
...
function get_maincats($catid, $moduleid, $level = -1) {
	...
	if($level >= 0) $condition .= " AND level=$level";
	$cat = array();
	$result = $db->query("SELECT catid,catname,child,style,linkurl,item FROM {$db->pre}category WHERE $condition ORDER BY listorder,catid ASC limit 6", 'CACHE');
	while($r = $db->fetch_array($result)) {
		$cat[] = $r;
	}
	return $cat;
...
function get_main_area(){
	global $db;
	$cat = array();
	$result = $db->query("SELECT areaname,areaid,listorder FROM {$db->pre}area  WHERE parentid = 0 ORDER BY rand() ASC limit 3",'CACHE');
	while($r = $db->fetch_array($result)) {
		$cat[] = $r;
	}
	return $cat;
}
function get_mall_area(){
	global $db;
	$cat = array();
	$result = $db->query("SELECT username,itemid,areaid FROM {$db->pre}mall group by areaid",'CACHE');
	while($r = $db->fetch_array($result)) {
		$cat[] = $r;
	}
	return $cat;
}
function get_area_good($areaid){
	global $db;
	$cat = array();
	$result = $db->query("SELECT * FROM {$db->pre}mall where areaid=$areaid",'CACHE');
	while($r = $db->fetch_array($result)) {
		$cat[] = $r;
	}
	return $cat;
...
//手机端主页楼层商品展示
function get_mobile_goods($catid,$areaid=0){
	global $db;
	$catid = explode(',',$catid);
	for($i=0;$i<count($catid);$i++){
		$childid[] = get_catid($catid[$i]);
	}
	$cat = array();
	for($i=0;$i<count($childid);$i++){

		if($areaid == 0){
			$sql = "SELECT itemid,areaid,title,thumb,price,sales FROM {$db->pre}mall WHERE catid in (".$childid[$i]['arrchildid'].") ORDER BY rand() limit 3";
		}else{
			$sql = "SELECT itemid,areaid,title,thumb,price,sales FROM {$db->pre}mall WHERE catid in (".$childid[$i]['arrchildid'].") AND areaid = $areaid";
		}
		$result = $db->query($sql,'CACHE');
		while($r = $db->fetch_array($result)){
			$r['thumb'] = str_replace('.thumb.'.file_ext($r['thumb']), '',$r['thumb']);
			$area = get_area($r['areaid']);
			$r['areaid'] = $area['areaname'];
			$r['price'] = floatval($r['price']);//保留有效数字
			$cat[$i][] = $r;
		}
	}
	return $cat;
...
// 猜你喜欢,根据最近一条浏览记录生成
function get_rand_goods(){
	global $db;
	if(isset($_COOKIE['item_his'])){
		...
		if($childid['arrchildid']){
			$sql = "SELECT * FROM {$db->pre}mall WHERE catid in (".$childid['arrchildid'].") limit 6";
		}else{
			$sql = "SELECT * FROM {$db->pre}mall WHERE catid = ".$catid['catid']." limit 6";
		}
	}else{
	    $sql = "SELECT * FROM {$db->pre}mall WHERE level <> 4 order by rand() desc limit 6";
	}
    $result = $db->query($sql,'CACHE');
    $pos = array();
    while($r = $db->fetch_array($result)) {
		$r['thumb'] = str_replace('.thumb.'.file_ext($r['thumb']), '',$r['thumb']);
		$r['title'] = dsubstr($r['title'],60,'...');
		$pos[] = $r;
	}
	return $pos;
}
// 生成浏览记录
function make_history($itemid){
	if(isset($_COOKIE['item_his'])){
		$his = explode(',',$_COOKIE['item_his']);
		if(in_array($itemid,$his)){
			$k = array_keys($his,$itemid);
			$his[$k[0]] = $his[0];
			$his[0] = $itemid;
			$itemid = implode(',',$his);
			setcookie('item_his',$itemid,time()+3600*24,'/');
		}else{
			array_unshift($his,$itemid);
			$itemid = implode(',',$his);
			setcookie('item_his',$itemid,time()+3600*24,'/');
		}
	}else{
...
function get_history($num=0){
	if($num == 1){
		...
		$his = $_COOKIE['item_his'];
		global $db;
	    $sql = "SELECT itemid,title,price,thumb FROM {$db->pre}mall WHERE itemid in ($his)";
	    $result = $db->query($sql,'CACHE');
	    $pos = array();
	    while($r = $db->fetch_array($result)) {
			$r['thumb'] = str_replace('.thumb.'.file_ext($r[thumb]), '',$r[thumb]);
...
// 清空浏览记录
function dele_his(){
	if(setcookie("item_his","", time()-3600,'/')){
		return 1;
	}
}
// 获取各种类型的订单
function get_mall_order($list){
	global $db;
	foreach($list as $k=>$v){
		$mall = $db->get_one("SELECT areaid FROM {$db->pre}mall WHERE itemid=".$v['mallid'],'CACHE');
		$area = $db->get_one("SELECT areaname FROM {$db->pre}area WHERE areaid=".$mall['areaid'],'CACHE');
		$list[$k]['areaid'] = $mall['areaid'];
		$list[$k]['area'] = $area['areaname'];
	}
	return $list;
}
function get_mobile_changguan(){
	global $db;
	$cat = array();
	$result = $db->query("SELECT username,changguan,areaid FROM {$db->pre}company WHERE groupid=6 ORDER BY rand() desc limit 3",'CACHE');
	while($r = $db->fetch_array($result)) {
		$cat[] = $r;
	}
	return $cat;
}
function get_cart_changguan($areaid) {
	global $db;
	$res = $db->get_one("SELECT areaname FROM {$db->pre}area WHERE areaid=$areaid");
	return $res;
}
function get_cart_cate($catid) {
	global $db;
	$res = $db->get_one("SELECT catid,parentid,catname FROM {$db->pre}category WHERE catid=$catid");
	if($res['parentid']){
		return $db->get_one("SELECT catid,parentid,catname FROM {$db->pre}category WHERE catid=".$res['parentid']);
	}else{
		return $res;
	}
}
// 最近搜索，存储
function make_search($kw){
	global $db,$_userid;
	$res = $db->get_one("SELECT userid,content FROM {$db->pre}search WHERE userid=$_userid AND content='$kw'");
	if($res){
		$db->query("UPDATE {$db->pre}search SET num=num+1,time=".time()." WHERE userid=$_userid AND content='$kw'");
	}else{
		$db->query("INSERT INTO {$db->pre}search(content,userid,num,time)VALUES('".$kw."',".$_userid.",1,".time().")");
	}
}
// 获取最近搜索和最热搜索
function get_search($ishot){
	global $db,$_userid;
	if($ishot){
		$cat = array();
		$res = $db->query("SELECT content FROM {$db->pre}search ORDER BY num DESC LIMIT 10");
		...
		$cat = array();
		$res = $db->query("SELECT content FROM {$db->pre}search WHERE userid=$_userid ORDER BY time DESC LIMIT 10");
		while($r = $db->fetch_array($res)) {
			$cat[] = $r;
		}
		return $cat;
...
// 获取新增收货地址第一层select
function get_address_area() {
	global $db;
	$areaid = intval($areaid);
	$are = array();
	$result = $db->query("SELECT areaid,areaname FROM {$db->pre}area WHERE parentid=0", 'CACHE');
	while($r = $db->fetch_array($result)) {
		$are[] = $r;
	}
	return $are;
}
function is_favorite($itemid){
	global $db,$_userid;
	$res = $db->get_one("SELECT * FROM {$db->pre}favorite WHERE userid=$_userid AND typeid=$itemid", 'CACHE');
	if($res){
		return true;
	}else{
		return false;
...
function api_goods(){
	global $db;
    $sql = "SELECT title,itemid,price,thumb FROM {$db->pre}mall WHERE level <> 4 order by rand() desc limit 5";
    $result = $db->query($sql,'CACHE');
    $pos = array();
    while($r = $db->fetch_array($result)) {
		$r['thumb'] = str_replace('.thumb.'.file_ext($r['thumb']), '',$r['thumb']);
		$r['alt'] = $r['title'];
		$r['title'] = dsubstr($r['title'],28,'...');
		$pos[] = $r;
	}
	return $pos;
}
function get_goods_comment($itemid){
	global $db;
    $sql = "SELECT * FROM {$db->pre}mall_comment WHERE mallid=$itemid ORDER BY seller_ctime DESC";
    $result = $db->query($sql);
    $pos = array();
    while($r = $db->fetch_array($result)) {
		$pos[] = $r;
	}
	$pos['all'] = $db->count($db->pre.'mall_comment', "mallid=$itemid");
	$pos['good'] = $db->count($db->pre.'mall_comment', "mallid=$itemid and seller_star=3");
	$pos['middle'] = $db->count($db->pre.'mall_comment', "mallid=$itemid and seller_star=2");
	$pos['bad'] = $db->count("{$db->pre}mall_comment", "mallid=$itemid and seller_star=1");
	return $pos;
}
// 会员整合方法
function tongbu($url,$post_data){
