...
if($action == 'update') {
	...
	switch($step) {
		...
		break;
		case 'delete'://订单删除
			$res = $db->query("DELETE FROM {$table} WHERE itemid=$itemid");
			if($res){
				dheader('trade.php?mobile=1&action=order');
			}else{
				message('收货未成功', $forward, 3);
			}
		break;
		case 'print'://订单打印
			$td['total'] = $td['amount'] + $td['fee'];
		...
		case 'comment'://交易评价
			...
			if($_username == $td['seller']) {
				...
				if($submit) {
					...
					// dheader('?action=update&step=detail&itemid='.$itemid);
				...
				if($submit) {
					...
					dheader('?mobile=1&action=update&step=detail&itemid='.$itemid);
					message($L['trade_msg_comment_success'], $forward);
				}
			}
			$head_title = '评论订单';
		break;
		case 'comment_detail'://评价详情b
			if($td['mid'] != 16) message($L['trade_msg_deny_comment']);
