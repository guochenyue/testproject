...
if($submit) {
	...
	if($do->add($post)) {
		$userid = $do->userid;
		// 同步注册
		$userinfo['member'] = $db->get_one("SELECT * FROM {$DT_PRE}member WHERE userid='$userid'");
		$userinfo['company'] = $db->get_one("SELECT * FROM {$DT_PRE}company WHERE userid='$userid'");
