...
switch($action) {
	...
	case 'truename':
		...
		if($submit) {
			...
			$db->query("INSERT INTO {$DT_PRE}validate (type,username,ip,addtime,status,editor,edittime,title,thumb,thumb1,thumb2,sfz) VALUES ('$action','$username','$DT_IP','$DT_TIME','2','system','$DT_TIME','$truename','$thumb','$thumb1','$thumb2','$sfz')");
	...
	case 'company':
		...
		if($submit) {
			...
			if ($thumb1 || $thumb2){
				if(!$thumb1) message($L['validate_company_image']);
				if(!$thumb2) message($L['validate_company_image']);
			}else{
			...
			$thumb1 = dhtmlspecialchars($thumb1);
			$thumb2 = dhtmlspecialchars($thumb2);
			$db->query("INSERT INTO {$DT_PRE}validate (type,username,ip,addtime,status,editor,edittime,title,thumb,thumb1,thumb2,frdb,xydm,yyzz,zzjg) VALUES ('$action','$username','$DT_IP','$DT_TIME','2','system','$DT_TIME','$company','$thumb','$thumb1','$thumb2','$frdb','$xydm','$yyzz','$zzjg')");
